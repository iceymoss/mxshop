...
func InitConfig() {
	...
	configFileName := fmt.Sprintf("user_srv/%s-por.yaml", configFilePrefix)
	if debug {
		//Users/feng/go/src/mxshop_srvs/user_srv/config-debug.yaml
		configFileName = fmt.Sprintf("user_srv/%s-debug.yaml", configFilePrefix)
	}

	//读取本地.yaml文件，从中获取nacos配置信息
	//实例化对象
	v := viper.New()
	//读取配置文件
